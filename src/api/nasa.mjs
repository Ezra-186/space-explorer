const KEY = import.meta.env.VITE_NASA_API_KEY || import.meta.env.VITE_NASA_KEY || "DEMO_KEY";
...
export async function getTodayAPOD() {
  ...
  try {
    ...
    throw new Error(`APOD invalid JSON (status ${res.status})`);
  }
...
export async function getRandomGalleryImage(query = "space", { peopleSafe = true } = {}) {
  ...
  try {
    ...
    throw new Error(`NASA Images invalid JSON (status ${res.status})`);
  }
  const items = json?.collection?.items ?? [];

  const HUMAN_WORDS = [
    "astronaut","portrait","people","person","crew","group","student","students",
    "class","training","administrator","press","ceremony","meeting","conference",
    "award","audience","self-portrait","selfie","team","family","gala","visit"
  ];
  const hasPeople = (data = {}, href = "", title = "", desc = "") => {
    ...
    return HUMAN_WORDS.some(w =>
      kws.includes(w) || t.includes(w) || d.includes(w) || h.includes(`/${w}/`)
    );
  };
  ...
  if (!candidates.length) {
    if (query !== "nebula") {
      return getRandomGalleryImage("nebula", { peopleSafe });
    }
